...
const ListadoCitas = () => {
  ...
  // Obtener las citas del state Global!
  const citas = useSelector( (state) => state.citas );
  
  // Mensaje condicional
  const mensaje = Object.keys(citas.citas).length === 0 ? 'No hay citas' : 'Listado de citas';
               ...
               citas.citas.map(cita => (
                <div key={cita.id} className="media mt-3">
                    <div className="media-body">
                        ...
                        <p className="card-text"><span>Nombre Dueño:</span> {cita.proopietario}</p>
